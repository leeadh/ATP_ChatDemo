    ...
    conn.on('data', function(message) {
        ...
        if(rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            try {
                ...
                if(data.type == 'pm') {
                    log('message', chalk.underline(clients[conn.id].un) + ' to ' + chalk.underline(data.extra) + ': ' + data.message);

                
                }else 
                    log('message', '[' + data.type.charAt(0).toUpperCase() + data.type.substring(1) + '] ' + chalk.underline(clients[conn.id].un) + ': ' + data.message);
                    console.log("===Message ====")
                    ...
                    console.log("INSERT INTO messages VALUES (" + "'"+ clients[conn.id].un + "'" + "," + "'"+ msg + "'"+ ", " + "TO_DATE("+ "'"+ date + "'"+ "," + "'dd-MM-yyyy HH24:MI:SS')," + 'NULL,' + "'"+ others +"'"+")")
                    console.log("===end message====")
                    addDB("INSERT INTO messages VALUES (" + "'"+ clients[conn.id].un + "'" + "," + "'"+ msg + "'"+ ", " + "TO_DATE("+ "'"+ date + "'"+ "," + "'dd-MM-yyyy HH24:MI:SS')," + 'NULL,' + "'"+ others +"'"+")",
                    function(err){
                        if (!err){
...
function updateUser(id, name) {
    if(name.length > 2 && name.length < 17 && name.indexOf(' ') < 0 && !utils.checkUser(clients, name) && name.match(alphanumeric) && name != 'Console' && name != 'System') {
        ...
        console.log("====logging in of user====")

        addDB("INSERT INTO users VALUES (" + "'"+ name + "'" + ")",
        function(err){
            if (!err){
                console.log("success inserted users")
            }else{
                console.log(err);
            }
        });
